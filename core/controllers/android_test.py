...
class InitializeAndroidTestDataHandlerTest(test_utils.GenericTestBase):
    ...
    def test_initialize_in_production_raises_exception(self) -> None:
        ...
        with assert_raises_regexp_context_manager, prod_mode_swap:
            self.post_json(
                "/initialize_android_test_data", {}, use_payload=False, csrf_token=None
            )
...
class AndroidActivityHandlerTests(test_utils.GenericTestBase):
    ...
    def test_get_different_versions_of_exploration_returns_correct_json(self) -> None:
        ...
        with self.secrets_swap:
            ...
            self.assertEqual(
                ...
                [{"id": "exp_id", "version": 2, "payload": new_exploration.to_dict()}],
            )
    ...
    def test_get_subtopic_returns_correct_json(self) -> None:
        ...
        with self.secrets_swap:
            self.assertEqual(
                ...
                [{"id": "topic_id-1", "version": 1, "payload": subtopic.to_dict()}],
            )
    ...
    def test_get_exploration_translation_without_version_and_language_code_fails(
        self,
    ) -> None:
        with self.secrets_swap:
            response = self.get_json(
                "/android_data?activity_type=exp_translations&"
                'activities_data=[{"id": "translation_id"}]',
                headers={"X-ApiKey": "secret"},
                expected_status_int=400,
            )
            self.assertEqual(
    ...
    def test_get_exploration_translation_without_version_fails(self) -> None:
        with self.secrets_swap:
            response = self.get_json(
                "/android_data?activity_type=exp_translations&"
                'activities_data=[{"id": "translation_id", "language_code": "es"}]',
                headers={"X-ApiKey": "secret"},
                expected_status_int=400,
            )
            self.assertEqual(
    ...
    def test_get_exploration_translation_without_language_code_fails(self) -> None:
        with self.secrets_swap:
            response = self.get_json(
                "/android_data?activity_type=exp_translations&"
                'activities_data=[{"id": "translation_id", "version": 1}]',
    ...
    def test_get_exploration_translation_with_both_params_succeeds(self) -> None:
        # Preparação do mock de tradução.
        translation_model = translation_models.EntityTranslationsModel.create_new(
            "exploration",
            "translation_id",
            1,
        ...
        with self.secrets_swap:
            response = self.get_json(
                "/android_data?activity_type=exp_translations&"
                'activities_data=[{"id": "translation_id", '
                '"version": 1, "language_code": "es"}]',
                headers={"X-ApiKey": "secret"},
                expected_status_int=200,
            )
            self.assertEqual(len(response), 1)
            self.assertEqual(response[0]["id"], "translation_id")
            self.assertEqual(response[0]["version"], 1)
            self.assertEqual(response[0]["language_code"], "es")
            self.assertIn("content_id_123", response[0]["payload"])
